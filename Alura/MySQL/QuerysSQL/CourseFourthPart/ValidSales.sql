...
SELECT X.NOME, X.MES_ANO, X.VOLUME_COMPRA, X.VOLUME_LIMITE,
CASE WHEN (X.VOLUME_LIMITE - X.VOLUME_COMPRA) < 0 THEN 'INVÁLIDA'
ELSE 'VÁLIDA'
END AS STATUS_COMPRA,
CONCAT(CAST(ROUND((1 - (X.VOLUME_LIMITE/X.VOLUME_COMPRA))*100,2)
AS CHAR(4)), '%') AS DIFERENCA_PERCENTUAL
FROM (SELECT TC.NOME, 
DATE_FORMAT(NF.DATA_VENDA, '%Y-%m') AS MES_ANO,
SUM(INF.QUANTIDADE) AS VOLUME_COMPRA,
MAX(TC.VOLUME_DE_COMPRA) AS VOLUME_LIMITE
